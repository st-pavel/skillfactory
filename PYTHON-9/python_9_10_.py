...
def shuffle_seed(array):
    ...
    #print(np.iinfo(seed_max_number).max)
    seed = np.random.randint(2 ** 32)
    seed_number = np.random.randint(0, seed, dtype=np.uint32)
    np.random.seed(seed)
...
def get_loto(num):
    ''' Функция формирует трехмерный массив из таблиц случайных чисел формы 5х5
    '''
    loto_array = np.random.randint(1, 101, size=(num, 5,5), dtype=int)
    # Заполняем каждую страницу лото набором уникальных случайных чиисел 5*5      
    for i in range(num):
        a = set()
        while len(a) < 5*5:
...
print(get_loto(3))
 ...
 # в задании 49.9.10, генерирует num полей для игры в лото, 
 ...
 # Я это и так сделал в предыдущем задании
